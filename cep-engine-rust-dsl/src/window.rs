...
impl WindowBuffer {
    ...
    /// Count entries matching a tag within a custom time period (used by DSL rules)
    pub fn count_within(&self, feeder_id: &str, tag: &str, secs: u64) -> usize {
        let now    = now_ms();
        let cutoff = now.saturating_sub(secs * 1_000);
        self.store
            .get(feeder_id)
            .map(|entries| {
                entries
    ...
    /// Clear all entries for a feeder (all buckets)
    ...
    /// Clear only entries matching a specific bucket tag for a feeder
    pub fn clear_bucket(&self, feeder_id: &str, tag: &str) {
        if let Some(mut entries) = self.store.get_mut(feeder_id) {
